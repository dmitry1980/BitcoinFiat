...
*Setup instructions for a Gitian build of Bitcoinfiat Core using a Debian VM or physical system.*
...
- [Building Bitcoinfiat Core](#building-bitcoinfiat-core)
...
Clone the git repositories for Bitcoinfiat Core and Gitian.
...
in the Bitcoinfiat Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bitcoinfiat Core
----------------

To build Bitcoinfiat Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bitcoinfiat Core repository.
...
```bash
    ...
    From https://github.com/dmitry1980/Bitcoinfiat
...
and the bitcoinfiat git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoinfiat/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitcoinfiat
SIGPATH=/some/root/path/bitcoinfiat-detached-sigs

./bin/gbuild --url bitcoinfiat=${BTCPATH},signature=${SIGPATH} ../bitcoinfiat/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinfiat-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinfiat-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinfiat-osx-build.assert
```
...
[bitcoinfiat/gitian.sigs](https://github.com/dmitry1980/gitian.sigs/) repository, or if that's not possible to create a pull
request.
